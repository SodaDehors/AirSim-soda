...
### 2026-06-05 — SNN-EEGNet 被试内评估 + 跨被试重跑 ✅

**改动:** 新建 `train/train_snn_within.py`（--all 跑 9 人），加 Chebyshev 4-40Hz 滤波。`train/train_snn.py` 重跑跨被试 T=16。
**依赖:** SpikingJelly (Fang et al., Science Advances 2023) — 北京大学开源 SNN 框架
**结果:**

| 评估方式 | SNN-EEGNet | EEGNet | Conformer |
|----------|-----------|--------|-----------|
| 被试内平均 | **61.8%** | 61.6% | 32.5% |
| 跨被试 (7→2) | **72.4%** | 62.8% | 74.9% |

**SNN 被试内 9 人明细 (vs EEGNet):**
被试 1: 53.5%(+0.4) 被试 2: 44.1%(-0.7) 被试 3: 53.1%(+2.8) 被试 4: 49.7%(0)
被试 5: 79.9%(-2.0) 被试 6: 50.0%(-9.4) 被试 7: 80.6%(-1.0) 被试 8: 75.0%(+0.3)
被试 9: **70.1%(+11.4 vs EEGNet!)**

**洞见:** SNN 是唯一在被试内和跨被试都表现稳定的模型（15K 参数）。Conformer 被试内崩（小数据不适），EEGNet 跨被试弱（架构浅）。SNN 两者兼得。下一步加论文预处理预期跨被试 77%+。

### 2026-06-04 — SNN-EEGNet (脉冲神经网络) 首次探索 ✅

**改动:** 新建 `train/train_snn.py`，用 SpikingJelly 的 LIF 脉冲神经元替换 ReLU。
**结果:** 被试 3 首次跑通，最佳 **51.7%**。证明脉冲神经元替换 ReLU 可行。
